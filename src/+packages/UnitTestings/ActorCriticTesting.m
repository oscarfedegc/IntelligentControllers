classdef ActorCriticTesting < handle
    methods (Access = public)
        function self = ActorCriticTesting()
            return
        end
        ...
        function run(self)
            ...
            model.setInitialStates(trajectories.getSamples(), deg2rad([-40 0 0 0]))
            
            pitchController = ControllerFactory.create(ControllerTypes.WavenetPID);
            yawController = ControllerFactory.create(ControllerTypes.WavenetPID);
            ...
            nnaType = NetworkList.ActorCritic;
            inputs = 3;
            outputs = 3;
            
            learningRates = [3e-8, 3e-8, 3e-8, 5e-8, 5e-5];
